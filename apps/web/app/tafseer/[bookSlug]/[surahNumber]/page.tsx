...
export default async function TafseerSurahPage({ params, searchParams }: PageProps) {
  ...
  const surahName  = surah?.metadata.nameEnglish ?? `Surah ${n}`;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10" style={{ paddingBottom: '100px' }}>
      ...
      <ReadingProgress
        ...
        label={surahName}
        breadcrumb={[
          { label: 'Tafseer', href: '/tafseer' },
          { label: book.name, href: `/tafseer/${book.id}` },
      ...
      <header style={{ marginBottom: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
          <div>
            {surah && (
              <>
                <div dir="rtl" lang="ar" style={{ fontFamily: 'var(--font-amiri)', fontSize: 'clamp(1.8rem, 4vw, 2.6rem)', color: 'var(--gold)', lineHeight: 1.6, marginBottom: '4px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  <span style={{ fontFamily: 'var(--font-lora)', fontStyle: 'italic', fontSize: '0.85rem', color: 'var(--ink-muted)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
            <span style={{ fontFamily: 'var(--font-lora)', fontSize: '0.78rem', color: book.accentColor, background: book.bgColor, border: `1px solid ${book.borderColor}`, borderRadius: '20px', padding: '3px 14px', whiteSpace: 'nowrap' }}>
              {book.name}
            </span>
            <Link href={`/quran/${n}`} style={{ fontFamily: 'var(--font-lora)', fontSize: '0.78rem', color: 'var(--gold)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      {/* ── Tafseer reader ───────────────────────────────────────────────── */}
      {tafseer && tafseer.entries.length > 0 && (
        <TafseerReader entries={tafseer.entries} surah={n} bookName={book.name} targetAyah={targetAyah} />
      )}

      {/* ── Surah navigation ─────────────────────────────────────────────── */}
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '56px', paddingTop: '24px', borderTop: '1px solid var(--gold-border)', gap: '16px' }}>
        {prev ? <NavLink href={`/tafseer/${book.id}/${prev}`} label="← Previous" sub={`Surah ${prev}`} /> : <div />}
        {next && <NavLink href={`/tafseer/${book.id}/${next}`} label="Next →" sub={`Surah ${next}`} align="right" />}
      </div>
...
function NavLink({ href, label, sub, align = 'left' }: { href: string; label: string; sub: string; align?: 'left' | 'right' }) {
  return (
    <Link href={href} style={{ fontFamily: 'var(--font-lora)', fontSize: '0.85rem', color: 'var(--gold)', textDecoration: 'none', padding: '10px 16px', borderRadius: '8px', border: '1px solid var(--gold-border)', background: 'var(--bg-card)', textAlign: align, maxWidth: '45%' }}>
