...
export default function Navigation() {
  ...
  return (
    <>
      <nav
        style={{ height: 'var(--nav-height)', background: 'rgba(13,17,23,0.92)', borderBottom: '1px solid var(--gold-border)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}
      ...
      >
        <div className="max-w-6xl mx-auto h-full px-4 sm:px-6 flex items-center justify-between gap-4">
          ...
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
            ...
            <span style={{ fontFamily: 'var(--font-cormorant)', fontSize: '1.15rem', fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.01em' }}>IQRA Digital</span>
          </Link>
          ...
          <button
            ...
            style={{ background: 'var(--bg-card)', border: '1px solid var(--gold-border)', color: 'var(--ink-muted)', borderRadius: '8px', padding: '6px 14px', fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontFamily: 'var(--font-lora)' }}
            aria-label="Open search"
          >
            <SearchIcon />
            <span className="hidden sm:inline">Search…</span>
            <span className="hidden sm:inline" style={{ fontSize: '0.7rem', padding: '1px 5px', borderRadius: '4px', background: 'rgba(200,168,75,0.1)', color: 'var(--gold)', border: '1px solid var(--gold-border)' }}>⌘K</span>
          </button>
        </div>
      </nav>
